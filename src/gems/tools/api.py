...
class FinancialStatementsInput(BaseModel):
    ticker: str = Field(
        description="The stock ticker symbol to fetch financial statements for. For Chinese stocks, use format like '000001.SZ' for A-shares."
    )
    period: Literal["annual", "quarterly"] = Field(
        description="The reporting period for the financial statements. 'annual' for yearly, 'quarterly' for quarterly. Note: 'ttm' not supported with AkShare."
    )
    limit: int = Field(
        default=10, description="The number of past financial statements to retrieve."
    )
    report_period_gt: str | None = Field(
        default=None,
        description="Optional fitler to retrieve financial statements greater than the specified report period.",
    )
    report_period_gte: str | None = Field(
        default=None,
        description="Optional fitler to retrieve financial statements greater than or equal to the specified report period.",
    )
    report_period_lt: str | None = Field(
        default=None,
        description="Optional fitler to retrieve financial statements less than the specified report period.",
    )
    report_period_lte: str | None = Field(
        default=None,
        description="Optional fitler to retrieve financial statements less than or equal to the specified report period.",
    )
...
class RealtimeDataInput(BaseModel):
    ticker: str = Field(
        description="股票代码，A股格式如'000001.SZ'，港股格式如'00700.HK'"
    )
