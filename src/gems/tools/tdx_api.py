...
class TDXProvider:
    ...
    def connect_ex(self) -> bool:
        ...
        for server_ip, server_port in self.ex_servers:
            try:
                ...
                logger.warning(
                    f"连接通达信扩展行情服务器 {server_ip}:{server_port} 失败: {e}"
                )
                continue
    ...
    def get_real_time_data(self, symbol: str) -> dict[str, Any] | None:
        """
        获取股票实时行情数据
...
def get_tdx_realtime_data(symbol: str) -> dict[str, Any] | None:
    """
    通过通达信API获取实时行情数据
...
def get_china_market_overview() -> str:
    ...
    try:
        ...
        for code, name in indices:
            ...
            if data:
                overview.append(
                    f"{name}: {data['current_price']} ({data['change_percent']:.2f}%)"
                )
