...
class Logger:
    ...
    def _add_file_handler(self, formatter: logging.Formatter):
        ...
        try:
            ...
            file_handler.setLevel(getattr(logging, config.LOG_LEVEL.upper(), logging.WARNING))
    ...
    def log_tool_call_error(self, tool_name: str, error: Exception, duration: float) -> None:
        """记录工具调用错误"""
        self.error(
            f"工具调用失败: {tool_name}",
            tool_name=tool_name,
    ...
    def log_llm_call_start(self, prompt: str, system_prompt: Optional[str] = None) -> None:
        """记录LLM调用开始"""
        self.debug(
            "LLM调用开始",
