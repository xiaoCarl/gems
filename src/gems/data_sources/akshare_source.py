    ...
    def get_financial_data(self, symbol: str, period: str) -> dict[str, Any]:
        ...
        try:
            ...
            if symbol.endswith(".HK"):
                hk_analysis_df = ak.stock_financial_hk_analysis_indicator_em(
                    symbol=clean_symbol
                )
                hk_financial_df = ak.stock_hk_financial_indicator_em(
                    symbol=clean_symbol
                )
                ...
                income_df = ak.stock_financial_report_sina(
                    stock=clean_symbol, symbol="利润表"
                )
                balance_df = ak.stock_financial_report_sina(
                    stock=clean_symbol, symbol="资产负债表"
                )
                cash_flow_df = ak.stock_financial_report_sina(
                    stock=clean_symbol, symbol="现金流量表"
                )
