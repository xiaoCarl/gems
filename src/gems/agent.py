...
class Agent:
    ...
    def ask_for_actions(self, task_desc: str, last_outputs: str = "") -> AIMessage:
        self.logger.debug("请求下一步行动", task_description=task_desc, last_outputs_length=len(last_outputs))
        ...
        try:
            ...
            response = call_llm(prompt, system_prompt=ACTION_SYSTEM_PROMPT, tools=tools_list)
            duration = time.time() - start_time
            self.logger.log_llm_call_end(response, duration)
            return response
        except Exception as e:
    ...
    def ask_if_done(self, task_desc: str, recent_results: str) -> bool:
        self.logger.debug("检查任务是否完成", task_description=task_desc, recent_results_length=len(recent_results))
        ...
        try:
            ...
            self.logger.debug("任务完成状态检查", task_description=task_desc, done=resp.done)
            return resp.done
        except Exception as e:
            duration = time.time() - start_time
            self.logger.log_llm_call_error(e, duration)
    ...
    def optimize_tool_args(self, tool_name: str, initial_args: dict, task_desc: str) -> dict:
        ...
        self.logger.debug("优化工具参数", tool_name=tool_name, task_description=task_desc)
    ...
    def _generate_answer(self, query: str, session_outputs: list) -> str:
        ...
        self.logger.debug("生成最终答案", query=query, session_outputs_count=len(session_outputs))
        ...
        start_time = time.time()
        try:
            self.logger.log_llm_call_start(answer_prompt, VALUE_INVESTMENT_ANSWER_PROMPT)
    ...
    def _confirm_stock_info(self, query: str):
        ...
        start_time = time.time()
        try:
            self.logger.log_llm_call_start(confirmation_prompt, STOCK_CONFIRMATION_PROMPT)
            # 使用LLM确认股票信息
            ...
            if confirmation.clarification_needed:
                self.logger.debug("需要澄清股票信息", clarification=confirmation.clarification_needed)
                print(f"   {confirmation.clarification_needed}")
                return False  # 需要重新输入
            else:
                self.logger.debug("股票信息确认成功", stock_name=confirmation.stock_name)
